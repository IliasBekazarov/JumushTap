...
export default function EditModal({ job, onClose, onSaved, showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Вакансия жаңыланды! ✅');
      onSaved();
      onClose();
    } catch {
      showToast('Ката болду');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-slate-800 border border-indigo-500/40 rounded-2xl p-6 w-full max-w-lg shadow-2xl my-4" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-5">
          <h2 className="text-white font-bold text-lg">Вакансияны өзгөртүү</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div><label className={lbl}>Дарек</label><input className={inp} value={form.address} onChange={e => up('address', e.target.value)} /></div>
          <div>
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-slate-300 text-sm">Договорная зарплата</span>
            </label>
          </div>
          {!form.is_negotiable && (
            <div className="grid grid-cols-2 gap-3">
              <div><label className={lbl}>Цена от</label><input type="number" className={inp} value={form.salary_from} onChange={e => up('salary_from', e.target.value)} /></div>
              <div><label className={lbl}>Цена до</label><input type="number" className={inp} value={form.salary_to} onChange={e => up('salary_to', e.target.value)} /></div>
            </div>
          )}
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white rounded-xl py-2.5 font-medium transition-colors text-sm">Жок кылуу</button>
